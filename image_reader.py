...
def image_scale(img, lab):
    """
    Randomly scales the image and label.
    ...
    # Randomly rescale the image and label
...
def image_crop(img, lab):
    """
    Randomly crops the image and label.
...
def read_images_from_disk(input_queue, is_training, img_mean):
    """
    ...
    if is_training:
        ...
        # Random scale
        img, lab = image_scale(img, lab)
        # Random crop
        img, lab = image_crop(img, lab)
        # Random_rotate:
...
class Reader(object):
    ...
    def __init__(self, coord, data_list, is_training=True, img_mean=img_mean):
        """
        Initialize a Reader.
        """
        self.coord = coord
        ...
        self.queue = tf.train.slice_input_producer([self.imgs, self.labs], shuffle=is_training)
        self.img, self.lab = read_images_from_disk(self.queue, is_training, img_mean)
